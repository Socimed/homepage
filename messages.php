...
try {
  ...
  if ($view == 'message') {
    ...
    /* get recipient */
    if (isset($_GET['uid'])) {
      $get_recipient = $db->query(sprintf("SELECT user_id, user_name, CONCAT(users.user_firstname,' ',users.user_lastname) as user_fullname FROM users WHERE user_id = %s", secure($_GET['uid'], 'int'))) or _error("SQL_ERROR_THROWEN");
