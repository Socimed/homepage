...
try {
  ...
  switch ($_GET['view']) {
    case '':
      ...
      // page header
      page_header(__($system['system_title']) . ' - ' . __("Movies"), __($system['system_description_movies']));
      break;
    ...
    case 'genre':
      ...
      // page header
      page_header(__($system['system_title']) . ' - ' . __("Movies") . ' - ' . __($genre['genre_name']), __($genre['genre_description']));
      break;
    ...
    case 'movie':
      ...
      if ($user->_logged_in) {
        ...
        if ($system['movies_permission'] != "everyone") {
          _error('PERMISSION');
        }
      }
