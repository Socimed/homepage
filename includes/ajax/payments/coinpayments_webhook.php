    ...
    switch ($_GET['handle']) {
      ...
      case 'subscribe':
        ...
        if (!isset($_GET['node_type']) || !in_array($_GET['node_type'], ['user', 'page', 'group'])) {
          $user->update_coinpayments_transaction($transaction['transaction_id'], "Error (400): Bad Reuqeust [node_type is not set]", '-1');
        }
        ...
        switch ($_GET['node_type']) {
          case 'user':
            // get user
            $node = $user->get_user($_GET['node_id']);
            if (!$node) {
              $user->update_coinpayments_transaction($transaction['transaction_id'], "Error (400): Bad Reuqeust [node is invalid or not exist]", '-1');
            }
            break;

          case 'page':
            // get page
            $node = $user->get_page($_GET['node_id']);
            if (!$node) {
              $user->update_coinpayments_transaction($transaction['transaction_id'], "Error (400): Bad Reuqeust [node is invalid or not exist]", '-1');
            }
            break;
          ...
          case 'group':
            // get group
            $node = $user->get_group($_GET['node_id']);
            if (!$node) {
              $user->update_coinpayments_transaction($transaction['transaction_id'], "Error (400): Bad Reuqeust [node is invalid or not exist]", '-1');
            }
            break;
        ...
        if ($payment) {
          /* subscribe to node */
          $user->subscribe($_GET['node_id'], $_GET['node_type'], $transaction['user_id']);
          /* update coinpayments transaction */
          $user->update_coinpayments_transaction($transaction['transaction_id'], __("Transaction complete successfully"), '2');
          /* notify the user */
          $user->post_notification(array('to_user_id' => $transaction['user_id'], 'from_user_id' => '1', 'action' => 'coinpayments_complete'));
