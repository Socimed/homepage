...
function initialize_modal() {
  ...
  if ($(".js_datetimepicker").length > 0) {
    $('.js_datetimepicker').datetimepicker({
      ...
      locale: $('html').data('lang').split("_", 1).toString() || 'en'
    });
  }
  // run Clipboard
  ...
  $('body').on('click', '.js_like-page, .js_unlike-page', function () {
    ...
    $.post(api['users/connect'], { 'do': _do, 'id': id }, function (response) {
      if (response.callback) {
        ...
        if (_do == 'page-like') {
          _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_unlike-page" data-id="' + id + '"><i class="fa fa-heart mr5"></i>' + __['Unlike'] + '</button>');
        } else {
          _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_like-page" data-id="' + id + '"><i class="fa fa-heart mr5"></i>' + __['Like'] + '</button>');
        }
      }
    }, "json")
      .fail(function () {
