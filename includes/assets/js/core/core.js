...
$(function () {
  ...
  $('body').on('click', '.js_payment-paypal', function () {
    ...
    if (data['handle'] == "subscribe") {
      data['node_id'] = _this.data('id');
      data['node_type'] = _this.data('node');
    }
    /* button loading */
  ...
  $('body').on('submit', '#razorpay_form', function (e) {
    ...
    if (form.find('input[name="billing_name"]').val() != '' && form.find('input[name="billing_email"]').val() != '' && form.find('input[name="billing_phone"]').val() != '') {
      ...
      if (data['handle'] == "subscribe") {
        data['node_id'] = form.find('input[name="id"]').val();
        data['node_type'] = form.find('input[name="node"]').val();
      }
      var options = {
        "key": razorpay_key,
  ...
  $('body').on('click', '.js_payment-wallet-monetization', function () {
    ...
    button_status(_this, "loading");
    /* post the request */
    $.post(api['payments/wallet'], { 'do': 'wallet_monetization_payment', 'node_id': _this.data('id'), 'node_type': _this.data('node') }, function (response) {
      /* button reset */
      button_status(_this, "reset");
      /* check the response */
      if (!response) return;
